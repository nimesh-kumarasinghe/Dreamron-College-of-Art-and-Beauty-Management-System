...
{
    public partial class AddEmployeeAttendance : Form
    {
        Database db;
        ...
        {
            DataTable dtable = db.getDatatable("select DISTINCT Attendance.ADate,Attendance.RoleType,Attendance.AStatus,Attendance.ATimeIn,Attendance.ATimeOut,Attendance.EmpID,Employee.EIniName from Attendance,Employee where ADate = '" + date_picker.Value.Year + "-" + date_picker.Value.Month + "-" + date_picker.Value.Day + "' AND Attendance.Empid IN (select EmpID from Employee);");
            ...
            var empid = from row in dtable.AsEnumerable() select row["EmpID"].ToString();
            var name = from row in dtable.AsEnumerable() select row["EIniName"].ToString();
            ...
            {
                addEmployeeAttendaceBindingSource.Add(new addEmployeeAttendace() { id = id, i_name = names[x] });
        ...
        private void date_picker_ValueChanged(object sender, EventArgs e)
        {
            ...
            if(dtable.Rows.Count == 0)
            {
                ...
                DataTable dt = db.getDatatable("select EmpID,EIniName from Employee;");
                var user_ids = from row in dt.AsEnumerable() select row["EmpID"].ToString();
                var users_names = from row in dt.AsEnumerable() select row["EIniName"].ToString();
        ...
        private void btn_add_Click(object sender, EventArgs e)
        {
            int counter = 0;
            int row_count = table_view.RowCount;
            String query = "insert into Attendance(ADate,RoleType,AStatus,ATimeIn,ATimeOut,EmpID)values ";
            String values = "";
            while (counter < row_count)
            {
                values += "('" + date_picker.Value.Year + "-" + date_picker.Value.Month + "-" + date_picker.Value.Day + "','Employee','" +
            ...
            try
            {
                ...
                MessageBox.Show("Attendance Added Sucessfully !! ");
            }
            catch (Exception ex)
            {
        ...
        private void btn_update_Click(object sender, EventArgs e)
        {
            ...
            while (counter < row_count)
            {
                query += "update Attendance set Attendance.ATimeIn = '" +
                    ...
                    table_view.Rows[counter].Cells[4].Value.ToString() + "' where Attendance.EmpID = '" +
        ...
        private void btn_delete_Click(object sender, EventArgs e)
        {
            ...
            String query = "delete from Attendance where Attendance.EmpID = '" + table_view.Rows[index].Cells[0].Value.ToString() + "' AND Attendance.ADate = '" + date_picker.Value.Year + "-" + date_picker.Value.Month + "-" + date_picker.Value.Day + "';";
